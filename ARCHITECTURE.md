...
| Q51 | The maintainer asked for a collapsible sidebar (2026-09-15). §7.5 fixes the desktop sidebar at `w-64`. What holds? | From `lg` the sidebar carries a toggle above the user menu ("Collapse sidebar" / "Expand sidebar", `aria-expanded`, `aria-controls="sidebar"`; `PanelLeftClose` / `PanelLeftOpen` through `DirIcon`, so they mirror right to left). Collapsed, it is `w-16` and shows the brand mark, each item's icon, the toggle and the user's initial; every label stays as `sr-only` text or `aria-label`, and a tooltip (`components/ui/tooltip.tsx`, Radix) repeats it at the reading end on hover and focus; group names become `sr-only` with a short rule between groups; the content's `lg:ps-64` becomes `lg:ps-16`. Width and padding transition in 200 ms (§7.14's reduced-motion rule ends them at once). The choice is `sidebarCollapsed` in `pallet.prefs.v1` (default `false`; anything but `true` reads as `false`); `boot-prefs.js` ignores it, because React draws the sidebar from the store on its first render. Below `lg` nothing changes: the navigation sheet always shows labels. `e2e/sidebar.spec.ts`. |
| Q52 | The maintainer asked for larger, more legible list tables with every column in view on the factory's screens (2026-09-15). §7.5 caps the content at `max-w-7xl` and sets lists in `text-sm`. What holds? | The content column is `max-w-[112rem]`: a 1920 px screen uses its full width (the eleven order columns fit with the sidebar open), wider screens stay bounded. `DataTable` sets its desktop table in `text-base`; `TableHead` is `h-12 px-4 text-sm` and `TableCell` `px-4 py-3.5` (both from `components/ui/table.tsx`, so every list and the order lines share them); a `wrap` column is `min-w-48 max-w-80`. The phone cards below `md`, the report tables (§7.3.15, their own header classes, print-sized) and the receipt keep their sizes. The user's text-size preference (Q50) still scales all of it. |
| Q53 | The maintainer asked for taller list rows and for the order filters to stay out of the way until wanted, behind a button beside "New order" (2026-09-15). §7.3.4 shows the filter bar always. What holds? | `TableHead` is `h-14` and `TableCell` `px-4 py-5` (a 64 px row at the default text size). `ListFilters` takes `open`: from `md`, `false` renders only the search box (and the date-range error under it), `true` the card, which carries `id="list-filters"`; below `md` the sheet button is unchanged. `FiltersToggle` (`list-controls.tsx`) is the header button: outline, `ListFilter` icon, "Filters", the active count as a badge like the phone button, `aria-expanded` and `aria-controls="list-filters"`, hidden below `md`. `/orders` renders it before "New order" and starts with the card hidden unless the address already carries a filter (`activeFilters > 0`), so a bookmarked filtered list shows what filters it. The state is the page's, not a preference: a fresh visit starts closed. Customers, items, drivers, users and history keep their always-visible filters (one or two each). |
| Q54 | The maintainer asked for the return card's edit and delete to be icons in its top corner (2026-09-15). §7.3.6 lists them as buttons. What holds? | On a standing return the header ends (`ms-auto`, so the top end in every direction) with two ghost icon buttons: edit (`Pencil`, a link to `/returns/new?orderId&replaceReturnId`) and delete (`Trash2`, red on hover). Each keeps its full name as `aria-label` and shows it in a tooltip on hover and focus (`components/app/tip.tsx`; `AppShell` wraps the whole frame in the Radix `TooltipProvider`, so any page may use it). Reversed returns show neither, as before. |
| Q55 | The maintainer asked for the order search box and the status tabs on one row, search at the reading start and the tabs at the end, with "All" as the first tab (2026-09-16). §7.3.4 puts the tabs above the filter bar and lists OPEN first. What holds? | One wrapping row above the filter card: `SearchBox` first, `Tabs` with `ms-auto` (the end in every direction); on a phone the row wraps, search full width and the tabs under it. The tabs read ALL, OPEN, SETTLED, CANCELLED; the default `status` stays OPEN. The filter card (Q53) then holds only the pickers, and the phone sheet button stands alone on its row. |
| Q56 | The maintainer found the on/off switches on the customers and drivers lists unfriendly (2026-09-16). §7.3 gives those lists `Switch` filters. What holds? | The yes-or-no list filters on `/customers` (open orders, archived), `/drivers` (archived) and `/items` (low stock, archived) are `FilterChoice` (`list-controls.tsx`): a two-segment control drawn like the order status tabs, the plain state first ("All" or "Active") and the narrowing one second ("With open orders", "Low stock", "Include archived"). Underneath it is `FilterSegment`, a Radix radio group named by the narrowing label, with the reading direction passed down for the arrow keys. The URL flags and the API params are unchanged. `FilterSwitch` stays for the stock report's options and the order page's "Show reversed". `customers.list.hasOpenOrders` and `items.list.lowStockOnly` lose their "Only"; `common.filters.all` and `common.filters.active` are new. |
| Q57 | The maintainer noted that `/orders` says "Orders" in the top bar and again as the page heading, and asked for one row: search and status tabs at the start, the Filters and New order buttons at the end (2026-09-16). §7.3.4 renders a `PageHeader`. What holds? | `/orders` renders no `PageHeader`; its `<h1>` is `sr-only`, so the page keeps a level-one heading for assistive technology and the route tests. One wrapping row holds `SearchBox`, the status `Tabs` and, pushed to the end with `ms-auto`, `FiltersToggle` and "New order"; the filter card (Q53) sits under it. Other list pages keep their visible heading until asked otherwise. |
| Q58 | The maintainer asked for one height for every text input, dropdown, autocomplete and date field on every page: first 52 px, then, finding that extreme, 48 px with the buttons on the same scale (2026-09-16). §7.12 / §7.15 size controls at `h-10` (`md:h-9`). What holds? | `Input`, `SelectTrigger`, the `EntityCombobox` trigger, the date field with its calendar button, the password field with its eye button, `Button` (`default`, `lg`, `icon` = `size-12`), the status tabs and `FilterSegment` are all `h-12` (48 px) at every width, in `text-base`; `Button size="sm"` stays `h-10` for inline helpers and paging; `Textarea` keeps `min-h-20`. Controls built on `Button` inherit the height rather than overriding it (an override would have to be repeated under `md:` to beat a variant). The phone target rule still holds because nothing got smaller. |
...
| Q63 | §14.6 asks for E1–E8 against the seeded database, but through M6 every Playwright test ran against a mocked API (`e2e/fixtures.ts` seals `/api`), so nothing checked that the web app and the API actually work together, and the E6 baselines did not exist. The demo seed is dated from today, so its figures move with the calendar. What holds? | A second suite, `apps/web/playwright.live.config.ts` over `apps/web/e2e-live/`, runs the production web build (`vite preview`, `/api` proxied through `PALLET_API_ORIGIN`) against the built API on `pallet_test`. Before the API starts, `e2e-live/serve.mjs` runs `migrate.js`, then `dist/scripts/e2e-reset.js` (empties every table and puts back the counter, settings and one admin who needs no password change; refuses production and any database not named `*_test`), then `seed-demo.js`. Ports 3197 (API) and 4197 (web), one worker, one retry in CI. Each test stages the records its assertions depend on through the API (`stage()` in `e2e-live/live.ts`) on top of the seed, so its figures do not move with the date or with another test; staged names carry a per-run tag, so a local rerun against a reused stack still finds exactly one. Controls are found by their Sorani text read from `ckb.json` (`sorani()`), so a renamed key fails loudly. E6 renders the viewport scrolled to the top, masking today's date and order numbers; it runs on Linux only (`-linux.png` baselines), and in CI `updateSnapshots: 'missing'` writes any baseline the repository lacks — failing that run, as Playwright fails a test whose baseline it had to write — and uploads them as the `e6-screenshots` artifact to commit; the first baselines were committed that way before the suite joined `main`. CI runs `pnpm test:e2e:live` after the mocked suite. The first real-API run found no application bug; breaking the `orders.cancel` check in the web build makes E8 fail. |
...
- R2 For every order, in batches of 1,000 by ascending id (Q60): load source rows, run `computeOrderTotals`, compare with stored cache columns (order and lines).
...
// ── health ──
interface HealthDto { status: 'ok'; db: 'ok'; disk: 'ok'; version: string }
```
...
#### `GET /api/health`
- **Access:** `@Public()`; `@SkipThrottle({ global: true })`.
- **Steps:** `SELECT 1` with a 2-second statement timeout; then `statfs(UPLOADS_DIR)` — the uploads volume shares the VPS disk with the database (§13.1), and a full disk stops every write while `SELECT 1` keeps answering (Q61).
- **Response:** 200 `HealthDto` (`version` = `APP_VERSION`); database failure → 503 `SERVICE_UNAVAILABLE` with `details.reason = 'database'`; fewer than 256 MiB free (`MIN_FREE_DISK_BYTES`), or an unreadable uploads directory → 503 with `details.reason = 'disk'`.
...
2. **`public/boot-prefs.js`** (plain ES2020, no imports, wrapped in `try { … } catch { }`):
   - reads `localStorage.getItem('pallet.prefs.v1')`, parses JSON, and validates each field against the allowed values (`language ∈ {ckb, ar, en}`, `theme ∈ {light, dark, system}`, `palette ∈ PALETTES`, `font ∈ FONT_FAMILIES`, `fontSize ∈ {sm, md, lg, xl}`); any invalid or missing field takes its default (`ckb`, `light`, `harbor`, `inter`, `md`) (Q50); `sidebarCollapsed` is left to React (Q51);
...
- **Components:** one row with the search input (`q`, debounced 300 ms) at the start and `Tabs` for status (ALL / OPEN / SETTLED / CANCELLED, Q55) at the end, above a `DataTable`; filter card: customer, driver and item `EntityCombobox`, payment type `Select`, `DateRangePicker`. From `md` the filter card is hidden behind a "Filters" header button until asked for (Q53); the search box stays.
...
- **Actions:** "Filters" (`FiltersToggle`, Q53), "New order" (`orders.create`), at the end of the toolbar row; no visible page heading (Q57). Row click → `/orders/$orderId`.
...
|---|---|---|
| `AppShell` | `app-shell.tsx` | Skip link "Skip to content" (first focusable, `sr-only focus:not-sr-only`); `≥ lg`: fixed sidebar `w-64` at inline-start (`border-e`) with nav items (icon + label, active item `bg-accent` plus `aria-current="page"`), a collapse toggle and the user menu at the bottom (display name, role; "My account", "Log out"); collapsed to `w-16` icons with tooltips on request, remembered in the preferences (Q51); `< lg`: top bar `h-14` with a menu button opening a `Sheet side="start"`, the page title and the user menu. `<main id="main" tabIndex={-1}>` wraps `<AnimatedOutlet/>` (section 7.14). Content max width `max-w-[112rem]` (Q52), padding `px-4 sm:px-6 lg:px-8`. |
...
`GRANTABLE_PERMISSION_KEYS` (33 keys), `ADMIN_ONLY_PERMISSION_KEYS` (3 keys), `PERMISSION_KEYS`, types `GrantablePermissionKey`, `AdminOnlyPermissionKey`, `PermissionKey`, `PERMISSION_DEPENDENCIES`, `isGrantablePermissionKey()`, `isAdminOnlyPermissionKey()`, `closePermissionSet()`, `missingPermissionDependencies()`. The module grouping `PermissionMatrix` shows (items, purchases, customers, drivers, orders, returns, payments, reports, audit) is the key prefix, derived in the component from `GRANTABLE_PERMISSION_KEYS` in their declared order; the tick/untick rule (a grant pulls in its dependencies, a revoke drops its dependants) is `togglePermission()` in `apps/web/src/lib/permission-selection.ts`, built on `PERMISSION_DEPENDENCIES` (Q61).
...
`dist/scripts/migrate.js` (idempotent, exits 0 on success): (1) run `prisma migrate deploy` (child process, `DATABASE_MIGRATE_URL`); (2) run `prisma db execute --file prisma/sql/grants.sql`; (3) seed in one transaction as owner: insert `factory_settings (id 1, factory_name 'Pallet Factory', phone '-', address '-')` if missing (the singleton `order_counter (id 1, last_number 0)` row is inserted by `constraints.sql` inside the migration); if `users` is empty create the admin from `ADMIN_*` (Argon2id hash, `role ADMIN`, `mustChangePassword true`) and write an audit `USER.CREATE` row with `user_id` null. `dist/scripts/healthcheck.js`: `GET http://127.0.0.1:${API_PORT}/api/health`, exit 0 on HTTP 200 within 4 s, or on 503 whose `details.reason` is `disk` (Q61: `caddy` waits for `api` healthy, so a low disk must not stop the site from serving reads; the uptime check still sees the 503), else 1. `dist/scripts/reconcile.js`: runs the reconciliation (§14.4 I14), prints `N differences`, exits 1 when N > 0.
...
10. Log in as the first admin → forced password change → Settings: factory name (Sorani), phone, address, logo. Delete `ADMIN_PASSWORD` from `.env` (Q61).
...
| File | Purpose |
|---|---|
| `incident-triage.md` | Where to start when the uptime monitor, the disk alert, the backup dead-man ping or Sentry fires, or `reconcile` reports differences |
...
### 14.6 End-to-end (Playwright, chromium, `locale: 'ckb'` preferences pre-set via `boot-prefs`; E1–E8 run live, Q63)
...
| E6 | RTL screenshots | `expect(page).toHaveScreenshot()` of E2–E4 final forms in `ckb` at 1280 × 800 and 390 × 844, `maxDiffPixelRatio: 0.01`; baselines committed under `apps/web/e2e-live/__screenshots__/` (generated on the CI runner image, Q63) |
...
Every e2e test uses the seeded demo data (`db:seed:demo`, deterministic seed) and resets the database before the suite. E1–E8 run in the live suite (`pnpm test:e2e:live`, `apps/web/playwright.live.config.ts`, `apps/web/e2e-live/`); the mocked suite (`pnpm test:e2e`, `apps/web/e2e/`) keeps the UI states a real API cannot stage cheaply (Q63).
