...
| Auth | @nestjs/jwt, @nestjs/passport, passport, passport-jwt, argon2, cookie-parser | 11.0.2, 11.0.5, 0.7.0, 4.0.1, 0.45.1, 1.4.7 |
...
| `X-Request-Id` | both | The API accepts an incoming value matching `^[A-Za-z0-9-]{8,64}$`, otherwise generates a UUID v4; it is echoed on every response, logged on every log line, stored in `audit_logs.request_id`, and returned as `requestId` in error bodies. |
...
| 4 | `AuthGuard` | `common/guards/auth.guard.ts` | Skips `@Public()`. Verifies the JWT (`HS256` only, secret `JWT_ACCESS_SECRET`, no clock tolerance), loads `users` + `user_permissions` by `sub` in one query, rejects per `AUTH_*` codes, attaches `req.auth: AuthContext`. |
...
- **Request:** `multipart/form-data`, exactly one file part named `file`; query `UploadCreateQuery = z.strictObject({ kind: z.enum(UPLOAD_KINDS) })`. Multer `memoryStorage()` with `limits: { fileSize: 5 * 1024 * 1024, files: 1, fields: 0, parts: 2 }`.
- **Steps:** (1) Multer `LIMIT_FILE_SIZE` → `UPLOAD_TOO_LARGE`; no file → `UPLOAD_MISSING_FILE`; (2) magic bytes: PNG `89 50 4E 47 0D 0A 1A 0A`, JPEG `FF D8 FF`, WebP `52 49 46 46 ?? ?? ?? ?? 57 45 42 50`; none match → `UPLOAD_TYPE_NOT_ALLOWED` (the client `Content-Type` and file name are ignored); (3) `sharp(buffer, { limitInputPixels: 40_000_000, failOn: 'error' })`, `metadata()` failure → `UPLOAD_INVALID_IMAGE`; (4) re-encode: `.rotate().resize({ width: W, height: W, fit: 'inside', withoutEnlargement: true }).webp({ quality: 82 })` with `W = 1024` for `ITEM_IMAGE`, `W = 600` for `FACTORY_LOGO` (sharp drops all metadata by default; `withMetadata` is never called); (5) `fileName = randomBytes(16).toString('hex') + '.webp'`; write to `${UPLOADS_DIR}/${fileName}` with flag `wx` and mode `0o640`; (6) insert `uploads` row (`width`, `height`, `size_bytes` of the output).
...
2. **`public/boot-prefs.js`** (plain ES2020, no imports, wrapped in `try { … } catch { }`):
   - reads `localStorage.getItem('pallet.prefs.v1')`, parses JSON, and validates each field against the allowed values (`language ∈ {ckb, ar, en}`, `theme ∈ {light, dark}`, `fontSize ∈ {sm, md, lg, xl}`); any invalid or missing field takes its default (`ckb`, `light`, `md`);
   - sets `document.documentElement.lang = language`, `dir = (language === 'en' ? 'ltr' : 'rtl')`;
   - toggles the class `dark` on `<html>` when `theme === 'dark'`;
...
| `/settings` | `_app/settings.tsx` | `_app` | `requireAdmin()` | `settings.title` |
...
#### 7.3.20 `/settings` Factory settings (admin only)
...
- **Preferences** (applied instantly and persisted to `localStorage` key `pallet.prefs.v1`; no server call):
  - language: `RadioGroup` with the native names `کوردی (سۆرانی)`, `العربية`, `English`;
  - theme: light / dark;
  - font size: four steps `sm`, `md`, `lg`, `xl`, shown as buttons with a sample "Aa" at each size.
...
- **Receipt and print:** the `_print` layout removes the `dark` class from `<html>` on mount and restores it on unmount. `@media print { :root { color-scheme: light; } }` forces light tokens for any printed page (including reports).
...
- Font stack (in `@theme`): `--font-sans: "Inter Variable", "Noto Sans Arabic Variable", ui-sans-serif, system-ui, sans-serif;`. Inter has no Arabic glyphs, so the browser takes Arabic-script characters from Noto Sans Arabic and Latin letters and digits from Inter. This keeps digits Western and consistent in every language.
...
│   └── favicon.svg
├── src/
│   ├── main.tsx                  # bootstrap (7.1)
│   ├── router.tsx                # router instance and typed context
...
│   ├── lib/
│   │   ├── api-client.ts         # apiFetch, apiUpload, ApiError (7.7)
...
| S8 | DB check on every request | Prisma | `JwtStrategy.validate({ sub, tv })` loads `user` + `permissions`; rejects with `AUTH_TOKEN_INVALID` when user missing, `isActive = false` or `tokenVersion ≠ tv`; passport `TokenExpiredError` → `AUTH_TOKEN_EXPIRED`; no header → `AUTH_REQUIRED` | `apps/api/src/common/guards/auth.guard.ts` |
...
| I5 | Upload intake | `multer` 2.x via `FileInterceptor('file', …)` | `storage: memoryStorage()`, `limits: { fileSize: 5 * 1024 * 1024, files: 1, fields: 2, parts: 3 }`; oversize → 413 `UPLOAD_TOO_LARGE`; no file → 400 `UPLOAD_MISSING_FILE` | `apps/api/src/modules/uploads/uploads.controller.ts` |
| I6 | Type by magic bytes | `sharp` 0.35.4 | `sharp(buffer, { failOn: 'error', limitInputPixels: 40_000_000 }).metadata()`; `format ∉ {'png','jpeg','webp'}` → 415 `UPLOAD_TYPE_NOT_ALLOWED`; sharp throws → 400 `UPLOAD_INVALID_IMAGE`. The client-sent MIME type and filename are ignored | `apps/api/src/modules/uploads/image-processor.ts` |
