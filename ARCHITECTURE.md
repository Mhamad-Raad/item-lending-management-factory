...
| Q58 | The maintainer asked for one height, 52 px, for every text input, dropdown, autocomplete and date field on every page (2026-09-16). §7.12 / §7.15 size controls at `h-10` (`md:h-9`). What holds? | `Input`, `SelectTrigger`, the `EntityCombobox` trigger, the date field with its calendar button (`size-13`) and the password field's eye button are `h-13` (52 px) at every width, in `text-base`; `Textarea` keeps `min-h-20`. The status tabs and `FilterSegment` are `h-13` too, so a toolbar row lines up with its search box. Buttons keep §7.15's `h-10 md:h-9` (icon buttons `size-10 md:size-9`); the phone target rule still holds because nothing got smaller. |
