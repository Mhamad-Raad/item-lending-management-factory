...
- **2d done**: catalogue web — `DataTable`, `MoneyInput`/`QuantityInput`, `DatePicker`/`DateRangePicker`, `EntityCombobox`, `/items*`, `/customers*` (profile tabs wait for M3/M4), `/drivers`, `/history` date range + admin user filter; `docs/rtl-audit.md` (§7.11.1 checklist). **M2 is built**; its milestone review comes next, then M3 (`docs/iterations.md` 3a).
