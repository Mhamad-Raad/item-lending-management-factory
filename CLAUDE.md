...
- **2a in progress** (branch `feat/2a-uploads-settings`): `POST /api/uploads` + public `GET /api/uploads/:fileName` (Q35 pipeline, per-user `UploadThrottleGuard`), `GET/PUT /api/settings` (Q36 no-op save), web `apiUpload`, `ImageUploadField` and `/settings`. S-19 is covered.
- Still open from M1: S-12 (permission denial over the whole endpoint catalogue) and S-14 (cost stripping on items and purchases) need 2b–2c; `/history` gains its date-range filter with the shared `DatePicker` in 2d.
