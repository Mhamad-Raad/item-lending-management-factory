...
- **Whole-system audit** (2026-09-17): gates green, no vulnerable deps, security review without high findings, and a load test on a 120,000-order database (every list under 150 ms; the activity report and the positions matrix are the heavy responses). The one thing that broke, `reconcile` above a few tens of thousands of orders, is fixed by Q60 (R2 walks orders in batches of 1,000 by id; `findLedgerDiscrepancies(prisma, { batchSize })`). The remaining audit items (pg pool size, `status='OPEN'` aggregates and partial indexes, report period cap, container memory limits, `COOKIE_SECURE` refinement, log/Sentry scrubbing, first-deploy guard, health check on disk-full, sourcemaps in the Caddy image, §8.2 drift) are the M7 backlog.
