...
- Next: **M2, iteration 2a** — uploads module and factory settings (`docs/iterations.md`). Still open from M1: S-12 (table-driven permission denial over the whole endpoint catalogue), S-14 (cost stripping on items and purchases) and S-19 (uploads) need the M2 endpoints before they can be written; `/history` gains its date-range filter when the shared `DatePicker` arrives in 2d.
