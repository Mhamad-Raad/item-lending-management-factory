...
| `button`        | [x]     | Icon spacing with `gap`, no `mr-2`/`ml-2`. Press to 98 % with motion's `whileTap`, or CSS `active:scale-[0.98]` when `asChild` (§7.14).                                                             |
...
| `dialog`        | [x]     | Close button at `end-4`; header `text-start`; footer uses `gap-2`. Opens and closes with motion (fade and 98 % scale); focus returns to the opener.                                                 |
...
| `sheet`         | [x]     | Bottom and inline-start sides; the start side slides in from the right in Kurdish and Arabic (motion, `x` from ±100 %); close button at `end-4`.                                                    |
